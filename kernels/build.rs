...
const KERNEL_FILES: [&str; 1] = ["index_select_api.cu"];
...
fn main() -> Result<()> {
    ...
    let build_dir = match std::env::var("CANDLE_INDEX_SELECT_BUILD_DIR") {
        Err(_) =>
        {
    ...
    // Static lib name: libindexselect.a
    let out_file = build_dir.join("libindexselect.a");
    ...
    println!("cargo:rustc-link-lib=indexselect");
