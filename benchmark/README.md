...
# Serving commands:
```
docker run --runtime nvidia --gpus all -e CUDA_VISIBLE_DEVICES="2" -v ~/.cache/huggingface:/root/.cache/huggingface     --env "HF_TOKEN=$HF_TOKEN"     -p 8000:8000     --ipc=host     vllm/vllm-openai:latest Anonymized-First-Author-review-only/Qwen3-Embedding-0.6B-auto --served-model-name qwen3vllm
```
